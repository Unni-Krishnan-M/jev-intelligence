"""Small shared helpers: stable ids, ISO timestamps, periods, JSON sanitising and Evidence records."""
...
def period_str(p: pd.Period) -> str:
    """Start date of a period (``YYYY-MM-01`` for months, the week's first day for weeks)."""
    if isinstance(p, pd.Period) and p.freqstr.startswith("M"):
        return month_str(p)
    return pd.Period(p).start_time.strftime("%Y-%m-%d")
