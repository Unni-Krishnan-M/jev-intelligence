...
from jev_ml.intel.common import fnum, month_str, short_hash, stable_id
from jev_ml.intel.config import FORECAST_VERSION, IntelConfig
...
def fc_holt(y: np.ndarray, horizon: int, cfg: IntelConfig) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
...
def run_model(
    name: str, y: np.ndarray, horizon: int, cfg: IntelConfig
...
def backtest_model(
    name: str, raw: np.ndarray, transform: str, cfg: IntelConfig, horizon: int, n_origins: int
) -> _Backtest | None:
    n = len(raw)
...
def step_quantiles(resid: np.ndarray, cfg: IntelConfig, horizon: int) -> tuple[np.ndarray, np.ndarray] | None:
...
def honest_coverage(bt: _Backtest, cfg: IntelConfig, horizon: int) -> float | None:
...
def select_and_backtest(
    raw: np.ndarray, transform: str, cfg: IntelConfig, horizon: int, n_origins: int
) -> dict[str, Any] | None:
...
def forecast_series(
    s: Series, cfg: IntelConfig, as_of_key: str, data_version: str, horizon: int | None = None
...
def forecast_all(
    ...
    out, states = [], {}
    for s in series:
        if not s.id.startswith(FORECAST_PREFIXES):
            continue
        res = forecast_series(s, cfg, as_of_key, data_version)
        if res is None:
            continue
